<?php
    ...
    $this->title = 'Müəllimlər';
?>

<ol class="breadcrumb">
    <li><a href="/admin-bs"> Əsas səhifə</a></li>
    <li class="active">Müəllimlər</li>
</ol>

<div class="news-ads-index">
    <?= Html::a('Yarat', ['/teachers/create'], ['class' => 'btn btn-success', 'style' => 'margin-bottom: 10px;']) ?>
                ...
                [
                    ...
                    'buttons' => [
                        'view' => function ($url, $model) {
                            $query = Teachers::find()->select('status')->where(['r_id' => $model->r_id])->one();
                            ...
                            return Html::a('<span class="glyphicon '.$class.' teachersStatus" data-rid="'.$model->r_id.'"></span>', $url, [
                                'title' => 'Status',
                            ]);
                        },
                    ...
                    'urlCreator' => function ($action, $model, $key, $index) {
                        if ($action === 'update') {
                            $url = '/admin-bs/teachers/update?id='.$model->r_id;
                            return $url;
                        }
                        if ($action === 'delete') {
                            $url = '/admin-bs/teachers/delete?id='.$model->r_id;
                            return $url;
                        }
                    }
                ],
        ...
        <div class="table-responsive">
            <ul id="sortable" class="list-group">
                <?php foreach ($itemsOrder as $itemOrder): ?>
                    <li id="id_<?= $itemOrder->r_id ?>" class="list-group-item"><?= $itemOrder->fullname ?></li>
                <?php endforeach; ?>
            </ul>
        </div>
        ...
        $(function() {
            ...
            btn_save.on('click', function(e) {
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    url: '/admin-bs/teachers/order-save',
            ...
            $('.teachersStatus').on('click', function() {
                ...
                $.ajax({
                    url: '/admin-bs/teachers/status-change',
                    type: 'GET',
