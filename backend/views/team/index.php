...
<div class="news-ads-index">
    <?= Html::a('Yarat', ['/team/create'], ['class' => 'btn btn-success', 'style' => 'margin-bottom: 10px;']) ?>
                ...
                [
                    ...
                    'buttons' => [
                        'view' => function ($url, $model) {
                            $query = Team::find()->select('status')->where(['r_id' => $model->r_id])->one();
                            ...
                            return Html::a('<span class="glyphicon '.$class.' teamStatus" data-rid="'.$model->r_id.'"></span>', $url, [
                                'title' => 'Status',
                            ]);
                        },
                    ...
                    'urlCreator' => function ($action, $model, $key, $index) {
                        if ($action === 'update') {
                            $url = '/admin-bs/team/update?id='.$model->r_id;
                            return $url;
                        }
                        if ($action === 'delete') {
                            $url = '/admin-bs/team/delete?id='.$model->r_id;
                            return $url;
                        }
                    }
                ],
    ...
    $this->registerJs("
        $(function() {
            ...
            $('.teamStatus').on('click', function() {
                ...
                $.ajax({
                    url: '/admin-bs/team/status-change',
                    type: 'GET',
